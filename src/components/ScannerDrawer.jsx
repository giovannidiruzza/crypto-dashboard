...
const ScannerDrawer = ({
  ...
  const [sortBy, setSortBy] = useState('gainers'); // 'gainers', 'losers', 'volume', 'alpha', 'favorites'
  ...
  const filteredTickers = useMemo(() => {
    ...
    return result;
  }, [tickers, search, sortBy, minVolume, favorites]);

  if (!isOpen) return null;

  return (
    <div className="fixed inset-y-0 left-0 z-50 w-80 sm:w-96 bg-[#080c14] border-r border-white/15 shadow-[10px_0_30px_rgba(0,0,0,0.8)] flex flex-col transition-all duration-300 animate-in slide-in-from-left">
      {/* Drawer Header */}
      <div className="p-3 border-b border-white/10 flex items-center justify-between bg-[#0e1420]">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded bg-amber-500/20 text-amber-400 flex items-center justify-center border border-amber-500/40">
            <Flame size={14} />
          </div>
          <div>
            <h2 className="text-sm font-bold text-white tracking-tight flex items-center gap-1 font-mono">
              Scanner Futures <span className="text-cyan-400 text-xs">Bitget</span>
            </h2>
            <p className="text-[10px] text-gray-400">
              {filteredTickers.length} contratti trovati
            </p>
          </div>
        </div>
      ...
      <div className="p-2.5 bg-gradient-to-r from-cyan-950/60 to-blue-950/60 border-b border-cyan-500/30 flex items-center justify-between gap-2">
        ...
        <button
          ...
          title="Riempi automaticamente gli slot della griglia con i primi token di questa lista"
      ...
      <div className="px-2 py-1.5 bg-[#070a10] border-b border-white/5 flex items-center gap-1 overflow-x-auto text-[10px]">
          ...
          filteredTickers.map((t, idx) => {
            ...
            return (
              <div
                ...
                className="px-3 py-2 flex items-center justify-between hover:bg-cyan-950/30 cursor-pointer transition-colors group select-none"
              >
